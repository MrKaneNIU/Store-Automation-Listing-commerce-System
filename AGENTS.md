...
`.ai/` 是本地 AI 工作流状态目录，不应被视为当前模块的实现 diff。

Reviewer 不应仅因为 `.ai/` 中存在未跟踪的工作流产物，就判定当前模块不通过。

在进行实现审查时，Reviewer 应重点关注与当前模块相关的已跟踪业务代码、源码、测试、配置文件 diff。

`.ai/TASK_STATE.md` 可以用于读取任务连续性信息，但除非用户明确要求，否则 `.ai/` 目录下的文件不属于模块验收范围。