...
describe('customer address page', () => {
  ...
  it('uses the existing customer mobile visual system with loading empty failure and retry states', () => {
    ...
    expect(source).toContain('class="add-address-entry"')
    expect(source).toContain('class="add-address-button"')
    expect(source).toContain('+新增地址')
    expect(source).toContain('v-if="isAddressSheetOpen"')
    expect(source).toContain('class="address-sheet-layer"')
    expect(source).toContain('class="address-sheet"')
    expect(source).toContain('class="form-card sheet-form"')
    expect(source).not.toContain('<view class="form-card">')
  ...
  it('opens add and edit address forms in the bottom sheet without changing facade commands', () => {
    expect(source).toContain('const isAddressSheetOpen = ref(false)')
    expect(source).toContain('openCreateAddressSheet')
    expect(source).toContain('openEditAddressSheet')
    expect(source).toContain('closeAddressSheet')
    expect(source).toContain('handleSaveAddress')
    expect(source).toContain('@tap="openCreateAddressSheet"')
    expect(source).toContain('@tap.stop="openEditAddressSheet(item.id)"')
    expect(source).toContain('@tap="handleSaveAddress"')
    expect(source).toContain('addressState.resetForm()')
    expect(source).toContain('addressState.editAddress(addressId)')
    expect(source).toContain('await addressState.saveAddress()')
  })
